...
//清洗篮维护
public class WuPinBaoActivity1 extends BaseActivity {
    ...
    public int setLayoutId() {
        return R.layout.activity_wu_pin_bao1;
    }
    ...
    public void initDatas() {
        ...
        if (biaoshi.equals("1")) {
            ...
            log.w("制包第一次进入物品包明细");
//            获取新生成物品包追踪记录 = "7630"; string 物品包id|领用科室id
        } else if (biaoshi.equals("2")) {
            log.w("2进入物品包明细");
            Bundle bundle = intent.getExtras();

            list_all_wpb = (List<Hsd_Mx>) bundle.getSerializable("list_huisdmx");//所有回收单里面的物品包集合，用于当前页面扫码 传回已扫描的
//            wupinbaoid = intent.getStringExtra("wupinbaoid");
            tiaoma = intent.getStringExtra("tiaoma");
            if (wupinbaoid != null) {
                tvSerach.setText(tiaoma);//条码号
                dialog.show();
                cachedThreadPool.execute(new Runnable() {
                    @Override
                    public void run() {
                        chaxun(wupinbaoid);
                        shengChengTiaoMa(tiaoma);//1；生成新条码  2；查询是否已经有图片
                    }
                });
            }
        }
    ...
    public void onViewClicked(View view) {
        ...
        switch (view.getId()) {
            case R.id.iv_back:
                ...
                break;
            case R.id.tv_right:
                String[] arry = fanArry(tiaoma);
                if (arry.length == 1) {     //首次拍照
                    if (tag == 1 || tag == 2) {
                        ToastUtils.showLong("不满足拍照条件");
                    } else {
                        takePotho();
                    }
                } else {                    //第二次拍照
    ...
    private String[] fanArry(String tiaoma) {
        String[] spilt = null;
//        String[] spilt =new String[]{};
        if (map_old_new_image.containsKey(tiaoma)) {
            String data = (String) map_old_new_image.get(tiaoma);
            spilt = data.split("#");
        }
        return spilt;
    }
            ...
            public void callBack(Hsms hsms) {
                ...
                if (hsms != null) {
                    ...
                    if (hsms.getReturnCode() == 0) {
                        ...
                        ToastUtils.showLong(Constant.NODATA + WSOpraTypeCode.获取物品包明细);
                    }
                } else {
                    ToastUtils.showLong(Constant.HSMS + WSOpraTypeCode.获取物品包明细);
                    log.e(Constant.HSMS);
                }
            }
        });
     ...
     * @param ma
     */
    private void shengChengTiaoMa(String ma) {
        HashMap<String, Object> params = new HashMap<>();
        String s = Constant.STRING + ma + "|" + userMsg.getID() + "|";
            ...
            public void callBack(Hsms hsms) {
                ...
                if (hsms != null) {
                    ...
                    if (hsms.getReturnCode() == 0) {
                        ...
                        QXBean b = list_saoma.get(0);
                        if (b.getID().equals("")) {
                            ToastUtils.showLong("无效的条码");
                            return;
                        }
                        log.e("生成新条码--" + b.getID());
                        String newma = b.getID();
                        String oldma = b.getOldZzjlID();
                        String image = b.getWupinBaoImage();

                        if (!map_ma.containsKey(oldma)) {  //保存新条码，用于绑定清洗蓝
                            map_ma.put(oldma, newma);
                        }
                        if (!map_old_new_image.containsKey(oldma)) {  //保存旧条码  照片  用于判断是否已拍照
                            map_old_new_image.put(oldma, newma + "#" + image);
                        }
                        log.e(map_ma.size() + "包   " + map_old_new_image.size());
                    } else {
                        ShowDialog.setShowErro(mContext, hsms.getResturnMsg() + "--" + hsms.getReturnID() + "--" + hsms.getReturnCode() + "--" + hsms.getReturnJson());
    ...
    private Map map_ma = new HashMap<>();//单个 或 批量扫描
            ...
            public void callBack(Hsms hsms) {
                ...
                if (hsms != null) {
                    ...
                    if (hsms.getReturnCode() == 0) {
                        ...
                        ShowDialog.setShowErro(mContext, hsms.getResturnMsg() + "--" + hsms.getReturnID() + "--" + hsms.getReturnCode() + "--" + hsms.getReturnJson());
                        ToastUtils.showLong("图片上传失败" + WSOpraTypeCode.物品包拍照保存);
                    }
                } else {
                    ToastUtils.showLong(Constant.HSMS + WSOpraTypeCode.物品包拍照保存);
                    log.e(Constant.HSMS);
                }
            }
        });
    ...
    private Map map_old_new_image = new HashMap<>();
    ...
    private void dialogUpImage(String tiaoma) {
        ...
        content.setText(tiaoma + "物品包以拍照上传，是否要替换？");
...
//        String imageName = "imageTest.jpg";
    ...
    private void beforeLan() {
        ...
        if (map_ma.size() <= 0) {
            ...
            String newTiaoMa = (String) map_ma.get(tiaoma);
            dialog.show();
            ...
            if (pici_ma.endsWith(",")) {
                pici_ma = pici_ma.substring(0, pici_ma.length() - 1);
            }
//            showLoading(qingqiu);
            dialog.show();
    ...
    private void getPiciToLan(String picima) {
        HashMap<String, Object> params = new HashMap<>();
        ...
        WebServiceUtils.callWebService(dialog, NetUtils.isNetAvailable(mContext), params, new WebServiceUtils.WebServiceCallBack() {
            ...
            public void callBack(Hsms hsms) {
                ...
                if (hsms != null) {
                    ...
                    if (hsms.getReturnCode() == 0) {
                        String result = hsms.getReturnJson();
                        List<QingXiJiLu> list_picilan = JSON.parseArray(result, QingXiJiLu.class);
...
//                        showLoading(qingqiu);
    ...
    //一个包一个蓝/2个篮   先获取 条码对应的清洗篮
    private void getQingXiLan(String qxlan, String xintiaoma) {
        HashMap<String, Object> params = new HashMap<>();
        String s = "";
            ...
            public void callBack(Hsms hsms) {
                ...
                if (hsms != null) {
                    ...
                    if (hsms.getReturnCode() == 0) {
                        ...
                        String lan = list_chalan.get(0).getQingXiLan();//用于判断篮子里是否空
                        log.e(lan + "---" + qixilan);
                        //判断批量  是否已绑定该篮
                        if (map_ma.size() > 1) {
                            tag = 1;//表明是批量扫描过篮子
    ...
    private void bangDingLan(final List<QingXiJiLu> list_pici) {  //已扫描的篮子或
        ...
        if (list_pici != null) {
            ...
            if (map_ma.size() > 1 || tag == 1) {   //批量操作
                ...
                if (map_ma.containsKey(tiaoma)) {
                    String xintiaoma = (String) map_ma.get(tiaoma);
            ...
            public void callBack(Hsms hsms) {
                ...
                if (hsms != null) {
                    ...
                    if (hsms.getReturnCode() == 0) {
                        ...
                        ShowDialog.setShowErro(mContext, hsms.getResturnMsg() + "--" + hsms.getReturnID() + "--" + hsms.getReturnCode() + "--" + hsms.getReturnJson());
                        ToastUtils.showLong(Constant.NODATA + WSOpraTypeCode.物品包绑定清洗篮);
                    }
                } else {
                    ToastUtils.showLong(Constant.HSMS + WSOpraTypeCode.物品包绑定清洗篮);
                    log.e(Constant.HSMS);
                }
            }
        });
...
//                showLoading(qingqiu);
     ...
     * @param lan
     */
    private void piciUpOne(String lan) {
        HashMap<String, Object> params = new HashMap<>();
                    ...
                    public void callBack(Hsms hsms) {
                        ...
                        if (hsms != null) {
                            ...
                            if (hsms.getReturnCode() == 0) {
                                ...
                                List<QingXiJiLu> list_pici = new ArrayList<>();//挑拣出来所要操作的批次
                                for (int i = 0; i < list_chalan.size(); i++) {
                                    Set<String> get = map_ma.keySet();
                                    for (String test : get) {
                                        String o = (String) map_ma.get(test);
...
//                        showLoading(qingqiu);
            ...
            public void callBack(Hsms hsms) {
                ...
                if (hsms != null) {
                    ...
                    if (hsms.getReturnCode() == 0) {
                        ...
                        ShowDialog.setShowErro(mContext, hsms.getResturnMsg() + "--" + hsms.getReturnID() + "--" + hsms.getReturnCode() + "--" + hsms.getReturnJson());
                        ToastUtils.showLong(Constant.NODATA + WSOpraTypeCode.物品包修改清洗篮);
                    }
                } else {
                    ToastUtils.showLong(Constant.HSMS + WSOpraTypeCode.物品包修改清洗篮);
                    log.e(Constant.HSMS);
                }
            }
        });
            ...
            public void callBack(Hsms hsms) {
                ...
                if (hsms != null) {
                    ...
                    if (hsms.getReturnCode() == 0) {
                        ...
                        ShowDialog.setShowErro(mContext, hsms.getResturnMsg() + "--" + hsms.getReturnID() + "--" + hsms.getReturnCode() + "--" + hsms.getReturnJson());
                        ToastUtils.showLong(Constant.NODATA + WSOpraTypeCode.物品包解绑清洗篮);
                    }
                } else {
                    ToastUtils.showLong(Constant.HSMS + WSOpraTypeCode.物品包解绑清洗篮);
                    log.e(Constant.HSMS);
                }
            }
        });
    ...
    private void piCi(String data) {
        if (tag == 1 || tag == 2) { //单个/批量对应的数据
            ...
            map_old_new_image.clear();//单个/批量条码  照片保存情况 清空
            tag = 0;
        }
        saoma(data);
    }
